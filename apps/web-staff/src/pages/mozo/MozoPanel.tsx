      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-8">
        <section>
          ...
          {pendientes.length === 0 ? (
            ...
            <div className="space-y-2">
              {pendientes.map((l) => (
                <div key={l.sesionId} className="bg-white rounded-xl border border-red-100 px-5 py-4 flex items-center justify-between shadow-sm">
                  <div>
                    <p className="text-sm font-bold text-gray-900">Mesa <span className="text-red-600">{l.mesaNumero}</span></p>
                    <p className="text-xs text-gray-400 mt-0.5">{l.recibitoEn.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                  <button onClick={() => marcarAtendido(l.sesionId)} className="px-4 py-1.5 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-700 transition-colors">
                    Atendido
                  </button>
                </div>
              ))}
        ...
        {atendidos.length > 0 && (
          <section>
            ...
            <div className="space-y-2">
              {atendidos.map((l) => (
                <div key={l.sesionId} className="bg-white rounded-xl border border-gray-100 px-5 py-3 flex items-center justify-between opacity-60">
