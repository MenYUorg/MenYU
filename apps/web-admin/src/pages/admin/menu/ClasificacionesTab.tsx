...
function ClasificacionRow({
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    setSaving(true)
    try { await onUpdate(c.id, editName.trim()); setEditing(false) }
    finally { setSaving(false) }
  }

  const handleCancel = () => { setEditName(c.nombre); setEditing(false) }
    ...
    <div className="flex items-center justify-between px-4 py-2.5 hover:bg-gray-50 group transition-colors">
      {editing ? (
        <form onSubmit={handleSave} className="flex items-center gap-3 flex-1">
          <Input value={editName} onChange={(e) => setEditName(e.target.value)} className="py-1 text-sm w-56" autoFocus />
...
export function ClasificacionesTab() {
  ...
  const { clasificaciones, loading, createClasificacion, updateClasificacion, deleteClasificacion, error } = useMenuStore()
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    setCreating(true)
    try { await createClasificacion(newName.trim()); setNewName('') }
    finally { setCreating(false) }
  }

  if (!selectedRestauranteId) {
    return <div className="flex items-center justify-center h-48 text-gray-400 text-sm">Seleccioná un restaurante para gestionar las clasificaciones</div>
  }

  return (
    <div>
      <p className="text-sm text-gray-500 mb-4">
        Las clasificaciones se asignan directamente a los ítems del menú. Usá texto libre: "Vegano", "Sin TACC", "Apto celíaco", "Keto", etc.
      </p>
      <form onSubmit={handleCreate} className="flex items-end gap-3 mb-6">
        <Input label="Nueva clasificación" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Ej: Vegano, Sin TACC, Bajo sodio..." className="w-72" />
        ...
        <div className="rounded-lg border border-gray-200 overflow-hidden bg-white">
          ...
          <div className="divide-y divide-gray-100">
            {clasificaciones.map((c) => (
              <ClasificacionRow key={c.id} c={c} onUpdate={updateClasificacion} onDelete={deleteClasificacion} />
            ))}
            {clasificaciones.length === 0 && <p className="text-sm text-gray-400 text-center py-12">No hay clasificaciones. Agregá la primera.</p>}
          </div>
        </div>
      )}
    </div>
