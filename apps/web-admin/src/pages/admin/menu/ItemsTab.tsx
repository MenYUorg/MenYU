...
export function ItemsTab() {
  ...
  const { items, categorias, clasificaciones, loading, createItem, updateItem, deleteItem, uploadItemImage, deleteItemImage } = useMenuStore()
  ...
  const handleUploadClick = (id: string) => { setImageTarget(id); fileRef.current?.click() }

  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file || !imageTarget) return
    try { await uploadItemImage(imageTarget, file) }
    catch { /* error queda en store */ }
    finally { setImageTarget(null); if (fileRef.current) fileRef.current.value = '' }
  }

  const handleDeleteImage = async (id: string) => {
    if (!window.confirm('¿Quitar la imagen de este ítem?')) return
    await deleteItemImage(id).catch(() => undefined)
  }

  const toggleClasificacion = async (clasificacionId: string) => {
    if (!editing) return
    setClasifSaving(clasificacionId)
    const yaAsignada = (editing.clasificaciones ?? []).some((ic) => ic.clasificacionId === clasificacionId)
    try {
      if (yaAsignada) {
        await api.clasificaciones.removeFromItem(editing.id, clasificacionId)
        setEditing((prev) => prev ? { ...prev, clasificaciones: (prev.clasificaciones ?? []).filter((ic) => ic.clasificacionId !== clasificacionId) } : prev)
      } else {
        await api.clasificaciones.addToItem(editing.id, clasificacionId)
        const nueva = clasificaciones.find((c) => c.id === clasificacionId)
        if (nueva) setEditing((prev) => prev ? { ...prev, clasificaciones: [...(prev.clasificaciones ?? []), { clasificacionId, clasificacion: nueva }] } : prev)
      }
    } catch { /* no bloqueamos el flujo principal */ }
    finally { setClasifSaving(null) }
  }

  if (!selectedRestauranteId) {
    return <div className="flex items-center justify-center h-48 text-gray-400 text-sm">Seleccioná un restaurante para ver los ítems</div>
  }

  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        <p className="text-sm text-gray-500">{items.length} {items.length === 1 ? 'ítem' : 'ítems'}</p>
        <Button onClick={openCreate} size="md">+ Nuevo ítem</Button>
      </div>
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="w-full text-sm bg-white">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      {item.imagenUrl && <Button variant="ghost" size="sm" onClick={() => handleDeleteImage(item.id)} title="Quitar imagen">🗑</Button>}
                      <Button variant="secondary" size="sm" onClick={() => setIngModal({ id: item.id, nombre: item.nombre })}>Ingredientes</Button>
              ...
              {items.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-16 text-center text-gray-400 text-sm">No hay ítems en el catálogo. Creá el primero.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal open={!!ingModal} onClose={() => setIngModal(null)} title={`Ingredientes — ${ingModal?.nombre ?? ''}`} size="lg">
        {ingModal && <ItemIngredientesPanel itemId={ingModal.id} itemNombre={ingModal.nombre} />}
      </Modal>

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar ítem' : 'Nuevo ítem'} size="md">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <Input label="Nombre *" value={form.nombre} onChange={(e) => setForm((f) => ({ ...f, nombre: e.target.value }))} required placeholder="Ej: Milanesa napolitana" />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {clasificaciones.map((c) => {
                  const asignada = (editing.clasificaciones ?? []).some((ic) => ic.clasificacionId === c.id)
                  const saving = clasifSaving === c.id
                  return (
                    <button key={c.id} type="button" disabled={saving} onClick={() => toggleClasificacion(c.id)}
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors disabled:opacity-50 ${asignada ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-300 hover:border-indigo-400 hover:text-indigo-600'}`}
                    >
          ...
          {formError && <p className="text-sm text-red-600 bg-red-50 border border-red-100 rounded-md px-3 py-2">{formError}</p>}
          <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
            <Button variant="secondary" type="button" onClick={() => setModalOpen(false)}>Cancelar</Button>
            <Button type="submit" loading={submitting}>{editing ? 'Guardar cambios' : 'Crear ítem'}</Button>
          </div>
        </form>
      </Modal>
    </div>
