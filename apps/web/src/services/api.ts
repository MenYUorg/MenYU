...
async function upload<T>(path: string, fieldName: string, file: File): Promise<T> {
  ...
  if (!res.ok) {
    const err = (await res.json().catch(() => ({}))) as Record<string, unknown>
    throw new ApiError(res.status, typeof err['message'] === 'string' ? err['message'] : `Error ${res.status}`)
  }
  return res.json() as Promise<T>
}
...
export const api = {
  auth: {
    ...
    register: (nombre: string, email: string, password: string) =>
      req<TokenPair>('POST', '/auth/register', { nombre, email, password }),
    guest: (nombre?: string) =>
      req<TokenPair>('POST', '/auth/guest', { nombre }),
    logout: (refreshToken: string) =>
      req<void>('POST', '/auth/logout', { refreshToken }),
  },

  menu: {
    publico: (restauranteId: string, params?: {
      categoriaId?: string
      buscar?: string
      dieta?: string[]
  ...
  sessions: {
    open: (data: { qrToken?: string; restauranteId?: string; pin?: string; codigoSesion?: string }) =>
      req<{ sesionId: string; esAnfitrion: boolean; codigoSesion: string }>('POST', '/sessions/open', data),
    close: (sesionId: string) =>
      req<void>('POST', '/sessions/close', { sesionId }),
  },
  ...
  items: {
    ...
    uploadImage: (id: string, file: File) => upload<ItemMenu>(`/items/${id}/imagen`, 'imagen', file),
