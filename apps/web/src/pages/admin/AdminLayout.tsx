...
export function AdminLayout() {
  const { user, marcas, restaurantes, selectedMarcaId, selectedRestauranteId, setMarca, setRestaurante, logout } = useAuthStore()
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4">
          <span className="text-sm font-semibold text-gray-800 mr-auto">Panel de administración</span>
          {marcas.length > 1 && (
            <Select
              value={selectedMarcaId ?? ''}
              onChange={(e) => { if (e.target.value) setMarca(e.target.value) }}
          ...
          {restaurantes.length > 1 && (
            <Select
              ...
              onChange={(e) => { if (e.target.value) setRestaurante(e.target.value) }}
              options={restaurantes.map((r) => ({ value: r.id, label: r.nombre }))}
              className="w-44 py-1.5 text-xs"
            />
          )}
          {marcas.length === 1 && (
            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">{marcas[0]?.nombre}</span>
          )}
          {restaurantes.length === 1 && (
            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">{restaurantes[0]?.nombre}</span>
          )}
        </header>
